...
export default async function TrackPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const response = await fetch(`${API_ORIGIN}/api/orders/track/${token}`, { cache: "no-store" });
  ...
  const paymentResponse = await fetch(`${API_ORIGIN}/api/orders/track/${token}/payment`, {
    cache: "no-store",
  });
